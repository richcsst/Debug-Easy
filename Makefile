# This Makefile is for the Debug::Easy extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Debug/Easy.pm]
#     AUTHOR => [q[Richard Kelsch <rich@rk-internet.com>]]
#     BUILD_REQUIRES => { Test::More=>q[0.44] }
#     CONFIGURE_REQUIRES => { Config=>q[0], ExtUtils::MakeMaker=>q[0], Term::ANSIColor=>q[0] }
#     MIN_PERL_VERSION => q[5.008]
#     NAME => q[Debug::Easy]
#     PL_FILES => {  }
#     PREREQ_PM => { Data::Dumper=>q[0], DateTime=>q[0], File::Basename=>q[0], Term::ANSIColor=>q[0], Test::More=>q[0.44], Time::HiRes=>q[0] }
#     PREREQ_PRINT => q[1]
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/Debug/Easy.pm]
#     clean => { FILES=>q[Debug-Easy-* _build*] }
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) -u \
	  lib/Debug/Easy.pm $(INST_MAN3DIR)/Debug::Easy.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''A Handy Debugging Module With Colorized Output and Formatting'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Richard Kelsch <rich@rk-internet.com>'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'build_requires:' >> META_new.yml
	$(NOECHO) $(ECHO) '  Test::More: '\''0.44'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Debug-Easy' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'version: '\''2.15'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'x_serialization_backend: '\''CPAN::Meta::YAML version 0.018'\''' >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) '{' > META_new.json
	$(NOECHO) $(ECHO) '   "abstract" : "A Handy Debugging Module With Colorized Output and Formatting",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Debug-Easy",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '            "Test::More" : "0.44"' >> META_new.json
	$(NOECHO) $(ECHO) '         }' >> META_new.json
	$(NOECHO) $(ECHO) '      },' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   "version" : "2.15",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="Debug-Easy" VERSION="2.15">' > Debug-Easy.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>A Handy Debugging Module With Colorized Output and Formatting</ABSTRACT>' >> Debug-Easy.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Richard Kelsch &lt;rich@rk-internet.com&gt;</AUTHOR>' >> Debug-Easy.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> Debug-Easy.ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,008,0,0" />' >> Debug-Easy.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Data::Dumper" />' >> Debug-Easy.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="DateTime::" />' >> Debug-Easy.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="File::Basename" />' >> Debug-Easy.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Term::ANSIColor" />' >> Debug-Easy.ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Time::HiRes" />' >> Debug-Easy.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.38" />' >> Debug-Easy.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> Debug-Easy.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> Debug-Easy.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> Debug-Easy.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/Debug/Easy.pm' 'blib/lib/Debug/Easy.pm' 
